 ...
 * a or b might be nil or null or None or nothing (except in Haskell, 
 * Elixir, C++, Rust, R, Shell, PureScript).
 * If a or b are nil (or null or None), the problem doesn't make 
...
export const comp = (a: number[] | null, b: number[] | null): boolean => {
  if (!a || !b) {
    return false;
  }

  const squaredA = a.map(n => n * n)

  return b.every(n => squaredA.includes(n))
};
